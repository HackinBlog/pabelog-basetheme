...
add_action( 'wp_enqueue_scripts', __NAMESPACE__ . '\add_custom_gutenberg_css', 15 );
/**
 * Outputs front-end inline styles based on colors declared in config/block-editor-settings.php.
 *
 * @since 2.9.0
 */
function add_custom_gutenberg_css() {
...
CSS;
	...
	$css .= add_inline_font_sizes();
	$css .= add_inline_color_palette();

	wp_add_inline_style( $prefix . '-gutenberg', $css );
...
CSS;
	...
	wp_add_inline_style( $prefix . '-gutenberg-fonts', $css );
 ...
 */
function add_inline_font_sizes() {
 ...
 */
function add_inline_color_palette() {
